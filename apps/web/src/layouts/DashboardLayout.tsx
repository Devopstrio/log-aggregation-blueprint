...
const DashboardLayout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const navItems = [
    { name: 'Observability Hub', path: '/', icon: LayoutDashboard },
    { name: 'Live Log Stream', path: '/stream', icon: Terminal },
    { name: 'Log Analytics', path: '/analytics', icon: BarChart3 },
    { name: 'Alerting Center', path: '/alerts', icon: Zap },
    { name: 'Security & SIEM', path: '/security', icon: ShieldCheck },
    { name: 'Retention & Lifecycle', path: '/retention', icon: History },
    { name: 'Infrastructure Health', path: '/infra', icon: Activity },
    { name: 'Audit & Compliance', path: '/audit', icon: Database },
    ...
    <div className="flex h-screen bg-slate-950 text-slate-200 overflow-hidden font-sans">
      ...
      <aside className={`bg-slate-900 border-r border-slate-800 transition-all duration-300 ${sidebarOpen ? 'w-64' : 'w-20'} flex flex-col`}>
        <div className="p-6 flex items-center gap-4 border-b border-slate-800">
          ...
          {sidebarOpen && <span className="font-bold text-xl tracking-tight bg-gradient-to-r from-white to-emerald-400 bg-clip-text text-transparent">Log.Aggregate</span>}
        </div>
        ...
        <div className="p-4 border-t border-slate-800">
          <div className="flex items-center gap-3 p-2 bg-slate-800/50 rounded-xl">
            <div className="w-8 h-8 rounded-full bg-emerald-600/20 flex items-center justify-center text-emerald-400 font-bold">
              OA
            </div>
            {sidebarOpen && (
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold truncate">Obs Architect</p>
        ...
        <header className="h-16 bg-slate-900/50 backdrop-blur-md border-b border-slate-800 flex items-center justify-between px-6 z-10">
          <div className="flex items-center gap-4 flex-1">
            ...
            <div className="relative max-w-md w-full">
              ...
              <input 
                type="text" 
                placeholder="Search logs, grep messages, filter service, analyze alerts..." 
          ...
          <div className="flex items-center gap-3">
            ...
            <button className="p-2 hover:bg-slate-800 rounded-lg text-slate-400 relative">
              <Bell size={20} />
              <span className="absolute top-2 right-2 w-2 h-2 bg-emerald-600 rounded-full border-2 border-slate-900"></span>
            </button>
          </div>
        </header>
